...
`Page-constructor` est une bibliothèque pour le rendu de pages web ou de leurs parties à partir de données `JSON` (le support du format `YAML` sera ajouté ultérieurement).
...
En interne, un package est utilisé pour transformer le Yandex Flavored Markdown en HTML - `diplodoc/transfrom`, il est donc également dans les dépendances pairées.
...
Le constructeur de pages vous permet d'utiliser des blocs définis par l'utilisateur dans leur application. Les blocs sont des composants React ordinaires.
...
7. Dans le répertoire du bloc, ajoutez une démo Storybook dans le dossier `__stories__`. Tout le contenu de démonstration pour l'histoire doit être placé dans `data.json` dans le répertoire de l'histoire. Le `Story` générique doit accepter le type des props du bloc, sinon des props de bloc incorrectes seront affichées dans Storybook.
...
Pour commencer à utiliser l'analytique, passez un gestionnaire au constructeur. Le gestionnaire doit être créé côté projet. Il reçoit trois classes d'événements :
```

- **Événements par défaut** : Ce sont des événements génériques du constructeur de page déclenchés pour les interactions avec les boutons, les liens, la navigation et les contrôles. Définissez `autoEvents.enabled` sur `true` pour les émettre.
- **Événements étendus** : Ce sont des événements enregistrés fournis par une bibliothèque composante. La présence de `autoEvents.extendedEvents` les active indépendamment de `enabled` et ajoute éventuellement un préfixe et un compteur.
- **Événements personnalisés** : Ils sont fournis par les consommateurs via `analyticsEvents`. La configuration des événements automatiques ne les modifie pas.

La forme objet est la configuration préférée :
...
La forme booléenne héritée reste prise en charge pour des raisons de compatibilité ascendante : `true` est équivalent à `{enabled: true}`, et `false` est équivalent à `{enabled: false}`. Si `autoEvents` est omis, les événements par défaut et étendus sont désactivés. Un objet `extendedEvents` active les événements étendus fournis même lorsque `enabled` est `false`.

Les événements étendus doivent avoir `type: 'extended-event'`. Leur préfixe est concaténé exactement comme configuré, sans modification de casse, de séparateurs ou d'espaces. Si `counter` est défini, il détermine `counters.include` pour l'événement étendu :

```ts
// Événement fourni
{name: 'REGISTERED_CLICK', type: 'extended-event'}

// Événement reçu par sendEvents avec la configuration ci-dessus
{
...
Les événements sont envoyés dans cet ordre : l'événement par défaut généré en premier (lorsqu'il est activé), suivi des événements étendus et personnalisés fournis dans leur ordre d'origine. Les événements étendus sont omis lorsque `extendedEvents` n'est pas configuré. Tout contexte supplémentaire spécifique à l'interaction est fusionné en dernier dans chaque événement émis.

Un objet événement n'a qu'un seul champ requis : `name`. Il possède également des champs prédéfinis, qui servent à gérer une logique complexe. Par exemple, `counter.include` peut aider à envoyer un événement dans un compteur particulier si plusieurs systèmes d'analyse sont utilisés dans un projet.
...
Il est possible de configurer un type d'événement nécessaire pour un projet.

```ts
type MyEventType = AnalyticsEvent<{
  [key: string]?: string; // seul un type 'string' est pris en charge
}>;
```

#### Sélecteur de compteur

Il est possible de configurer un événement auquel un système d'analyse doit être envoyé.

```ts
type AnalyticsCounters = {
  include?: string[]; // tableau d'identifiants de compteurs d'analyse qui seront appliqués
  exclude?: string[]; // tableau d'identifiants de compteurs d'analyse qui ne seront pas appliqués
};
```
...
enum PredefinedEventTypes {
  ...
  Extended = 'extended-event', // événements fournis par une bibliothèque composante
...
Dans les cas habituels, nous utilisons deux types de commits :
...
Lorsque vous recevez l'approbation de votre pull-request par les propriétaires du code et que toutes les vérifications sont passées, veuillez suivre ces étapes :

1. Vérifiez s'il existe une pull-request de publication du robot avec des modifications d'un autre contributeur (elle ressemble à `chore(main): release 0.0.0`). Si elle existe, vérifiez pourquoi elle n'a pas été fusionnée. Si le contributeur accepte de publier une version partagée, suivez l'étape suivante. Sinon, demandez-lui de publier sa version, puis suivez l'étape suivante.
...
4. Vous verrez également un champ pour la version manuelle. Si vous publiez une alpha dans votre branche pour la première fois, ne laissez rien ici. Après la première publication, vous devrez définir la nouvelle version manuellement car nous ne modifions pas `package.json` au cas où la branche pourrait expirer très bientôt. Utilisez le préfixe `alpha` dans votre version manuelle, sinon vous obtiendrez une erreur.
...
Si vous souhaitez publier une nouvelle version majeure, vous aurez probablement besoin de versions bêta avant une version stable. Veuillez suivre ces étapes :
...
4. Le robot de publication créera une nouvelle PR vers la branche `beta` avec CHANGELOG.md mis à jour et incrémentera la version du package.
5. Vous pouvez répéter cela autant de fois que vous le souhaitez. Lorsque vous serez prêt à publier la dernière version majeure sans tag bêta, vous devrez créer une PR de la branche `beta` vers la branche `main`. Notez qu'il est normal que la version de votre package soit taguée comme bêta. Le robot le sait et la modifiera correctement. `3.0.0-beta.0` deviendra `3.0.0`.

### Flux de publication pour les versions majeures précédentes

Si vous souhaitez publier une nouvelle version dans une version majeure précédente après l'avoir validée dans `main`, veuillez suivre ces étapes :

1. Mettez à jour la branche nécessaire. Les noms des branches de publication majeures précédentes sont :
   1. `version-1.x.x/fixes` - pour la version majeure 1.x.x
   2. `version-2.x.x` - pour la version majeure 2.x.x
2. Créez une nouvelle branche à partir de la branche de publication majeure précédente.
3. Cherry-pick votre commit de la branche `main`.
4. Créez une PR, obtenez une approbation et fusionnez-la dans la branche de publication majeure précédente.
5. Squash and merge votre PR (Il est important de publier une nouvelle version avec Github-Actions).
6. Attendez que le robot crée une PR avec une nouvelle version du package et des informations sur vos modifications dans CHANGELOG.md. Vous pouvez voir le processus dans l'onglet [Actions](https://github.com/gravity-ui/page-constructor/actions).
7. Vérifiez vos modifications dans CHANGELOG.md et approuvez la PR du robot.
8. Squash and merge la PR. Vous pouvez voir le processus de publication dans l'onglet [Actions](https://github.com/gravity-ui/page-constructor/actions).
...
Ce projet comprend une **Banque de mémoire** complète - une collection de fichiers de documentation Markdown qui fournissent des informations détaillées sur l'architecture, les composants et les modèles d'utilisation du projet. La Banque de mémoire est particulièrement utile lorsque vous travaillez avec des agents IA, car elle contient des informations structurées sur :

- **Vue d'ensemble du projet** : Exigences de base, objectifs et contexte
...
La Banque de mémoire est située dans le répertoire `memory-bank/` et se compose de fichiers Markdown ordinaires qui peuvent être lus comme toute autre documentation :

- `projectbrief.md` - Document fondamental avec les exigences de base
...
Une documentation complète est disponible sur le [lien fourni](./test-utils/docs/README.md).
...
Une bibliothèque pour le rendu de pages Web entières ou de sections de page à partir d'une configuration déclarative JSON/YAML, en utilisant un ensemble de blocs prêts à l'emploi et ordonnables — utilisez-la pour créer des pages marketing/de destination, pas des interfaces utilisateur d'application générales.

### Quand l'utiliser

- Pages basées sur les données : affichez une configuration de `content` de blocs typés avec `PageConstructor` encapsulé dans `PageConstructorProvider`.
- Pages marketing, de destination et de documentation assemblées à partir de blocs préfabriqués (en-têtes, médias, cartes, etc.).
- Traitement côté serveur de YFM du texte des blocs via les utilitaires `@gravity-ui/page-constructor/server` (`contentTransformer`, `fullTransform`).
...
- Coques de navigation d'application (en-tête latéral) — utilisez [`@gravity-ui/navigation`](https://github.com/gravity-ui/navigation) ; le composant `Navigation` de ce package est une navigation supérieure au niveau de la page.

### Pièges courants

- **`PageConstructor` doit être encapsulé dans `PageConstructorProvider`.** Le rendre sans encapsulage casse le contexte (locale, thème, SSR, analytique).
- **La prop `content` est `content`, structurée `{blocks: [...]}`.** Chaque objet de bloc doit avoir un `type` correspondant à un bloc connu plus ses champs de données ; il n'y a pas de prop `data`/`config`.
- **YFM dans le texte des blocs nécessite un traitement côté serveur.** Les champs de type Markdown s'affichent comme du texte brut à moins que vous ne traitiez le contenu via `contentTransformer`/`fullTransform` de `@gravity-ui/page-constructor/server` ; `@diplodoc/transform` est une dépendance pair requise.
...
- **Vite nécessite `vite-plugin-dynamic-import`.** Les importations dynamiques de blocs échouent sous Vite sans cela.

## Documentation pour les agents IA

La documentation lisible par l'agent pour la version installée se trouve dans `node_modules/@gravity-ui/page-constructor/build/docs/INDEX.md`.