...
在你的入口文件中导入一次 `@gravity-ui/uikit` 样式，用 `ThemeProvider` 包裹你的应用，并在一个具有明确高度的容器内渲染 `Chart`：
...
`Chart` 会自适应其父容器的大小，因此包裹元素必须有一个高度。
...
一个声明式的 React 图表库，适用于 Gravity UI 应用 — 从单一的 `data` 配置渲染折线图、面积图、柱状图、饼图、散点图、树状图等图表，并根据应用的主题进行样式设置。

### 何时使用

- 标准业务图表：`line`、`area`、`bar-x`/`bar-y`、`pie`、`scatter`、`treemap`、`waterfall`、`sankey`、`radar`、`heatmap`、`funnel`、`x-range`。
- 必须遵循 Gravity UI 主题（浅色/深色）并与 `@gravity-ui/uikit` 应用共享 token 的可视化。
- 从声明式数据渲染图表，而不是命令式绘制。

### 何时避免使用

- 仍在使用 `@gravity-ui/chartkit` 的项目 — 这是旧的基于适配器的包装器（YAGR/Highcharts/D3）；此包是现代的独立渲染器，并非直接替换。
...
- **`data` prop 是 `data`，结构为 `{series: {data: [...]}}`。** `series.data` 中的每个条目代表一个系列，它有自己的 `type` 和 `data` 数组 — 没有顶层的系列数组。
- **没有尺寸容器则什么都不会渲染。** `Chart` 会填充其父容器，因此请为包裹元素指定明确的高度。
