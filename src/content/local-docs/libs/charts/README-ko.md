...
10가지 이상의 차트 유형을 제공하는 React 차트 라이브러리: 영역, 막대, 선, 파이, 산점도, 트리맵 등.
...
`@gravity-ui/uikit`는 필수 피어 종속성입니다. 테마 및 스타일을 제공하여 차트가 의존합니다.

## 사용법

진입점에서 `@gravity-ui/uikit` 스타일을 한 번 가져오고, 앱을 `ThemeProvider`로 감싸고, 명시적인 높이를 가진 컨테이너 내부에 `Chart`를 렌더링하세요.
...
`Chart`는 부모 요소의 크기에 맞춰 조정되므로, 래핑하는 요소에는 높이가 지정되어야 합니다.
...
Gravity UI 앱을 위한 선언형 React 차트 라이브러리 — 단일 `data` 구성에서 선, 영역, 막대, 파이, 산점도, 트리맵 및 기타 차트를 렌더링하며 앱의 나머지 부분과 일치하도록 테마가 적용됩니다.
...
- 아직 `@gravity-ui/chartkit`을 사용하는 프로젝트 — 이는 이전 어댑터 기반 래퍼(YAGR/Highcharts/D3)이며, 이 패키지는 최신 독립형 렌더러이며 드롭인 대체품이 아닙니다.
- 일반 테이블 형식 데이터 — [`@gravity-ui/table`](https://github.com/gravity-ui/table)을 사용하세요.
- React가 아니거나 서버 전용 렌더링 — `Chart`는 React SVG를 렌더링하며 DOM이 필요합니다.

### 일반적인 함정

- **컴포넌트 이름은 `Chart`이며 `ChartKit`이 아닙니다.** `@gravity-ui/charts`에서 `{Chart}`를 가져오세요. `ChartKit`은 별도의 레거시 `@gravity-ui/chartkit` 패키지에 속합니다.
- **`data` 속성은 `{series: {data: [...]}}` 형식입니다.** `series.data`의 각 항목은 자체 `type`과 `data` 배열을 가진 하나의 시리즈입니다. 최상위 배열은 없습니다.
- **크기가 지정되지 않은 컨테이너 없이는 아무것도 렌더링되지 않습니다.** `Chart`는 부모를 채우므로 래퍼에 명시적인 높이를 지정하세요.
- **uikit 설정이 필요합니다.** `ThemeProvider`로 감싸고 `@gravity-ui/uikit/styles/styles.css`를 가져오세요. `@gravity-ui/uikit`는 필수 피어 종속성입니다.
...
- [축 유형](./docs/diplodoc/pages/guides/axes/axis-types.md)
- [축 레이블](./docs/diplodoc/pages/guides/axes/axis-labels.md)
