...
- [`docs/app-error.md`](https://github.com/gravity-ui/nodekit/blob/main/docs/app-error.md) contém a descrição de uma classe de erro personalizada útil que o NodeKit fornece para seus aplicativos
...
Um kit de ferramentas fundamental para Node.js (logging, telemetria, erros tipados, config, contextos de requisição) compartilhado entre os backends do Gravity UI — use-o para obter uma espinha dorsal de aplicativo consistente antes de adicionar qualquer camada HTTP, em vez de montar você mesmo a infraestrutura de logging/erros/configuração.
...
- **Lançar `Error` simples** — use o `AppError` empacotado (veja `docs/app-error.md`) para que os códigos de erro e a telemetria sejam capturados de forma consistente.
