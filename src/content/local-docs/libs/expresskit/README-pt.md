...
ExpressKit é um wrapper leve para [express.js](https://expressjs.com/) que se integra com [NodeKit](https://github.com/gravity-ui/nodekit) e oferece recursos úteis como log de requisições, suporte a tracing, controllers e middlewares assíncronos, e descrição detalhada de rotas.
...
| ------------------- | -------------------- | ------------------------------------ | ----------------------------------------------------------------------------------------------- |
| `appCsrfSecret`     | `string \| string[]` | -                                    | **Obrigatório.** Chave(s) secreta(s) para geração de tokens HMAC. Múltiplas chaves permitem rotação. |
...
const nodekit = new NodeKit({
  config: {
    ...
    // Certifique-se de que seu middleware defina o ID do usuário no originalContext, caso contrário, a geração do token CSRF falhará
