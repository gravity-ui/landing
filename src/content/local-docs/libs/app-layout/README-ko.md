...
렌더 함수는 플러그인으로 확장될 수 있습니다. 플러그인은 사용자 정의 렌더링 콘텐츠를 재작성할 수 있습니다.
...
app.get((req, res) => {
  res.send(
    renderLayout({
      ...
      pluginsOptions: {
        googleAnalytics: {
          useBeaconTransport: true, // navigator.sendBeacon 사용을 활성화합니다.
          counter: {
            id: 'some id',
          },
        },
...
`generateRenderContent`, `renderHeadContent`, `renderBodyContent` 파트 렌더러를 사용하여 HTML 스트리밍으로:
...
이 라이브러리는 React 애플리케이션을 위한 UI 컴포넌트 모음입니다.
...
`@gravity/uikit`은 테마를 지원합니다. 테마를 설정하려면 `ThemeProvider` 컴포넌트를 사용하세요.

```javascript
import React from 'react';
import ReactDOM from 'react-dom';
import { ThemeProvider, Button } from '@gravity/uikit';
...
기여하고 싶으신가요? [CONTRIBUTING.md](CONTRIBUTING.md) 파일을 참고해주세요.

## 라이선스

MIT 라이선스 하에 배포됩니다. 자세한 내용은 [LICENSE](LICENSE) 파일을 참고해주세요.
```